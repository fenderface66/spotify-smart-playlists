/*
 * HomePage Messages
 *
 * This contains all the text for the HomePage component.
 */
import { defineMessages } from 'react-intl';

export default defineMessages({
  startProjectHeader: {
    id: 'smartify.containers.HomePage.start_project.header',
    defaultMessage: 'Start your next react project in seconds',
  },
  startProjectMessage: {
    id: 'smartify.containers.HomePage.start_project.message',
    defaultMessage: 'A highly scalable, offline-first foundation with the best DX and a focus on performance and best practices',
  },
  trymeHeader: {
    id: 'smartify.containers.HomePage.login.header',
    defaultMessage: 'Log in to your spotify!',
  },
  trymeMessage: {
    id: 'smartify.containers.HomePage.tryme.message',
    defaultMessage: 'Show Github playlistsitories by',
  },
  trymeAtPrefix: {
    id: 'smartify.containers.HomePage.tryme.atPrefix',
    defaultMessage: '@',
  },
});
